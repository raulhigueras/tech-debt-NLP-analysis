...
def download_data_from_github(folder_path):
    """This fucntion checks whether the .db files are in data/raw; 
        if they are, it does nothing; 
        else, it downloads the data from github.
    The downloading process can take a while.
    """
    raw_files = os.listdir(folder_path)
    # if we already have the files, don't download
    if VERSION1 in raw_files and VERSION2 in raw_files: return
    ...
    with zipfile.ZipFile(f"{folder_path}/{VERSION1}.zip", 'r') as zip_ref:
        ...
        zip_ref.extract(f"{folder_path}/{VERSION1}")
...
def create_fts_table(cursor, version=2):
    """This function creates a FTS3 table in the database from cursor to index the comits by message and easily filtrate
        by this column. The process is inline.
    """
...
def read_sql_table_from_connection(query, connection, selected_columns=None):
    """This function executes a SQL query in the given connection. Although you can specify the columns selected in the query, 
        you can specify the columns needed in selected_columns. This will help to deal with different versions of the dataset.
    """
...
def add_changes_metrics(df, connection):
    """This function joins the data from the jira_issues table with the data from the FTS3 table. It gets, for each issue,
    the number of lines_added, lines_removed and files_changed.
    """
    out_df = df.copy()
...
def main(input_filepath, output_filepath):
    ...
    select_columns = ["key", "project_id", "creation_date", "resolution_date", "summary", "description", "type"] # jira columns
